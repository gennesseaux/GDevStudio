...
bool MenuFunctions::AddMenuItem(HMENU hTargetMenu, const CString& itemText, UINT itemID)
{
	...
	else
	{
		...
		else
		{
			...
			if (::AppendMenu(hTargetMenu, MF_SEPARATOR, itemID, itemText) > 0)
			{
	...
	int *pInsertPositions)			// can be NULL
{
	...
	else
	{
		...
		else
		{
			...
			bSuccess = (::InsertMenu(hTargetMenu, position, MF_BYPOSITION | MF_SEPARATOR, itemID, _T("")) > 0);
		}
	}	
...
bool MenuFunctions::AddSeparator(HMENU hTargetMenu)
{
	ASSERT(::IsMenu(hTargetMenu));
...
{
	while(::DeleteMenu(hMenu,0,MF_BYPOSITION) != FALSE);
	return true;
}
