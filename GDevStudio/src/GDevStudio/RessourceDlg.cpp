...
// Constructeur
CRessourceDlg::CRessourceDlg(GDSAPI::CRessource* pRessource, CWnd* pParent /*=NULL*/) : CGDevStudioDialogBase(CRessourceDlg::IDD, pParent)
{
...
void CRessourceDlg::OnEmplacementChange()
{
	...
	// Récupération du fichier si celui-ci est valide
	m_pRessource->SetFile(m_pRcReader->GetFichierRc());
...
void CRessourceDlg::OnAjouter()
{
	...
	for (int i = 0; i < m_pRcDialog->GetControlCount() ; i++)
	{
		...
		RcReader::CRcResourceControl* pRcControl = m_pRcDialog->GetControl(i);

		// Ajout du control à la ressource
		GDSAPI::CControle* pControl = new GDSAPI::CControle(DefULong,m_pRessource);
		pControl->SetLibelle(pRcControl->GetId());
		pControl->SetType(pRcControl->GetControle().ToString());
